...
export interface SterlingCandidateInput {
  ...
  ssn?:        string;  // full SSN – transmitted to Sterling, never stored
  zipCode?:    string;
  phone?:      string;
}
...
export interface SterlingWebhookPayload {
  ...
  candidateId: string;
  status:      string;
  result?:     string;                      // CLEAR | CONSIDER | DISPUTE
  reportId?:   string;
  completedAt?: string;
  [key: string]: unknown;
}
...
async function sterlingFetch<T>(path: string, init?: RequestInit): Promise<T> {
  const url = `${BASE_URL}${path}`;
  const res  = await fetch(url, { ...init, headers: { ...sterlingHeaders(), ...(init?.headers ?? {}) } });

  if (!res.ok) {
    const body = await res.text().catch(() => '(no body)');
    logger.error(`Sterling API ${init?.method ?? 'GET'} ${path} → ${res.status}: ${body}`);
...
export const sterlingService = {
   ...
   */
  async initiateCheck(input: SterlingCandidateInput): Promise<SterlingOrderResult> {
    if (!this.isConfigured()) {
      // Sandbox / dev fallback – return a fake order so devs can test the flow
      if (IS_SANDBOX || process.env.NODE_ENV !== 'production') {
        logger.warn('Sterling not configured – returning mock order for development');
        return {
    ...
    const candidate = await sterlingFetch<{ id: string }>('/candidates', {
      method: 'POST',
      body: JSON.stringify({
    ...
    // Step 2 – submit order
    const order = await sterlingFetch<{
      id:           string;
      candidate_id: string;
      status:       string;
      report_url?:  string;
    ...
    logger.info(`Sterling order created: ${order.id} for candidate ${order.candidate_id}`);
  ...
  async getOrderStatus(orderId: string): Promise<SterlingStatusResult> {
    if (!this.isConfigured()) {
      // Dev mock: after 30 s pretend it passed
      return { orderId, status: 'PENDING' };
    }

    const data = await sterlingFetch<{
      id:           string;
      status:       string;
      result?:      string;
      report_id?:   string;
  ...
  verifyWebhookSignature(rawBody: string, signatureHeader: string): boolean {
    if (!STERLING_WEBHOOK_SECRET) {
      if (process.env.NODE_ENV !== 'production') {
        logger.warn('Sterling webhook secret not set – skipping signature check in dev');
        return true;
      }
      logger.error('Sterling webhook secret not configured in production');
      return false;
    }
 ...
 */
function mapSterlingStatus(status: string, result?: string): SterlingResult {
