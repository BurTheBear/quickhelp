...
exports.backgroundCheckController = {
     ...
     */
    async devSimulate(req, res, next) {
        try {
            ...
            if (!req.user)
                throw new errorHandler_js_1.AppError('Not authenticated', 401);
            const userId = req.user.id;
            const result = (req.query.result ?? 'clear').toLowerCase();
            if (!['clear', 'consider'].includes(result)) {
                throw new errorHandler_js_1.AppError('result must be "clear" or "consider"', 400);
            }
            ...
            const check = await database_js_1.prisma.backgroundCheck.upsert({
                ...
                create: {
                    ...
                    expiresAt: status === 'CLEAR'
                        ? new Date(now.getTime() + 365 * 24 * 60 * 60 * 1000)
                        : null,
                    rawResult: { simulated: true, result },
                },
                update: {
                    status,
                    completedAt: now,
            ...
            logger_js_1.logger.info(`[DEV] Background check simulated for user ${userId} → ${status}`);
            res.json({
                success: true,
                data: {
                    id: check.id,
