...
exports.gamificationService = {
    async awardXP(userId, amount, reason, referenceId) {
        try {
            const gamification = await database_js_1.prisma.userGamification.upsert({
                where: { userId },
            ...
            await database_js_1.prisma.xPEvent.create({
                data: {
            ...
            if (level !== oldLevel) {
                await database_js_1.prisma.userGamification.update({
                    where: { userId },
            ...
            logger_js_1.logger.error(`Failed to award XP to ${userId}:`, err);
        }
    },
    async updateStreak(userId) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const streak = await database_js_1.prisma.userStreak.upsert({
            where: { userId },
            create: { userId, currentStreak: 1, longestStreak: 1, lastActivityDate: today },
            update: {},
        });
        if (!streak.lastActivityDate) {
            await database_js_1.prisma.userStreak.update({
                where: { userId },
    ...
    async checkBadgeEligibility(userId) {
        ...
        const badges = await database_js_1.prisma.badge.findMany({ where: { isActive: true } });
        for (const badge of badges) {
            ...
            if (eligible) {
                await database_js_1.prisma.userBadge.create({ data: { userId, badgeId: badge.id } });
    ...
    async updateChallengeProgress(userId, category) {
        const now = new Date();
        const activeChallenges = await database_js_1.prisma.challenge.findMany({
            where: {
                isActive: true,
                startDate: { lte: now },
                endDate: { gte: now },
        ...
        for (const challenge of activeChallenges) {
            const progress = await database_js_1.prisma.userChallengeProgress.upsert({
            ...
            if (progress.progress >= challenge.goal && !progress.completedAt) {
                await database_js_1.prisma.userChallengeProgress.update({
                    where: { userId_challengeId: { userId, challengeId: challenge.id } },
