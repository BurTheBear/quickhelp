...
exports.matchService = {
    async acceptRequest(requestId, volunteerId) {
        ...
        const bgCheck = await database_js_1.prisma.backgroundCheck.findUnique({
            where: { userId: volunteerId },
            select: { status: true, expiresAt: true },
        });
        const isCleared = bgCheck?.status === 'CLEAR' &&
        ...
        if (!isCleared) {
            ...
            throw new errorHandler_js_1.AppError('A cleared background check is required to volunteer.', 403, true, { requiresBackgroundCheck: true, checkStatus: statusMsg });
        }
        // ─────────────────────────────────────────────────────────────────────────
        ...
        if (!request)
            throw new errorHandler_js_1.AppError('Request not available', 404);
        if (request.authorId === volunteerId)
            throw new errorHandler_js_1.AppError('Cannot volunteer for your own request', 400);
        // Check if already matched by this volunteer
        const existingMatch = await database_js_1.prisma.match.findUnique({
            where: { requestId_volunteerId: { requestId, volunteerId } },
        });
        if (existingMatch)
            throw new errorHandler_js_1.AppError('Already matched with this request', 409);
        // Check volunteer has no conflicting active tasks
        const activeMatch = await database_js_1.prisma.match.findFirst({
            where: { volunteerId, status: { in: ['ACCEPTED', 'IN_PROGRESS'] } },
        });
        if (activeMatch)
            throw new errorHandler_js_1.AppError('You already have an active task in progress', 409);
        // Create match and update request status atomically
        const [match] = await database_js_1.prisma.$transaction([
            database_js_1.prisma.match.create({
                data: {
                    requestId,
                    volunteerId,
                    status: 'ACCEPTED',
            ...
            database_js_1.prisma.helpRequest.update({
                where: { id: requestId },
                data: { status: 'MATCHED' },
            }),
        ]);
        ...
        await notification_service_js_1.notificationService.create(request.authorId, {
    ...
    async startTask(matchId, volunteerId) {
        const match = await database_js_1.prisma.match.findFirst({
            where: { id: matchId, volunteerId, status: 'ACCEPTED' },
            include: { request: { select: { authorId: true, title: true } } },
        });
        if (!match)
            throw new errorHandler_js_1.AppError('Match not found', 404);
        const updated = await database_js_1.prisma.$transaction([
            database_js_1.prisma.match.update({
                where: { id: matchId },
        ...
        await notification_service_js_1.notificationService.create(match.request.authorId, {
            type: 'TASK_STARTED',
    ...
    async completeTask(matchId, volunteerId) {
        const match = await database_js_1.prisma.match.findFirst({
        ...
        if (!match)
            throw new errorHandler_js_1.AppError('Match not found', 404);
        const now = new Date();
        const [updatedMatch] = await database_js_1.prisma.$transaction([
            database_js_1.prisma.match.update({
                where: { id: matchId },
            ...
            database_js_1.prisma.userProfile.update({
                where: { userId: volunteerId },
        ...
        await gamification_service_js_1.gamificationService.awardXP(volunteerId, match.request.rewardPoints, `Completed: ${match.request.title}`, matchId);
        // Update streak
        await gamification_service_js_1.gamificationService.updateStreak(volunteerId);
        // Update challenge progress
        await gamification_service_js_1.gamificationService.updateChallengeProgress(volunteerId, match.request.category);
    ...
    async requestCompletion(matchId, volunteerId) {
        const match = await database_js_1.prisma.match.findFirst({
        ...
        if (!match)
            throw new errorHandler_js_1.AppError('Match not found', 404);
        const updatedMatch = await database_js_1.prisma.match.update({
            where: { id: matchId },
    ...
    async approveCompletion(matchId, requesterId) {
        const match = await database_js_1.prisma.match.findFirst({
        ...
        if (!match)
            throw new errorHandler_js_1.AppError('Match not found', 404);
        if (match.request.authorId !== requesterId)
        ...
        await gamification_service_js_1.gamificationService.updateStreak(volunteerId);
        await gamification_service_js_1.gamificationService.updateChallengeProgress(volunteerId, match.request.category);
        await notification_service_js_1.notificationService.create(volunteerId, {
            type: 'TASK_COMPLETED',
            title: 'Task Approved! 🎉',
    ...
    async cancelMatch(matchId, userId, reason) {
        ...
        await notification_service_js_1.notificationService.create(notifyUserId, {
    ...
    async getActiveMatches(userId) {
        return database_js_1.prisma.match.findMany({
            where: {
                volunteerId: userId,
                status: { in: ['ACCEPTED', 'IN_PROGRESS'] },
            },
